...
const Shop = () => {
  ...
  const listItemsToBuy = () => items.map((item) => (
    <div key={item.id} className="card">
      <div key={item.id} className="font-text"><h2>{`${item.name}`}</h2></div>
      <h2 key={item.id}>{`Price: $${item.price}`}</h2>
      <img key={item.id} src={item.image} style={{ width: "200px", height: "300px", objectFit: "cover" }} className="book-avatar" />
      <h2 key={item.id}>Likes: {item.likes}</h2>
      <div>
      <button key={item.id} onClick={()=> updateLike(item)}> Like </button>
