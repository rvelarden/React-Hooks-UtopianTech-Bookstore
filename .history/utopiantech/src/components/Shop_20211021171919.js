...
const Shop = () => {
  ...
  const listItemsToBuy = () => items.map((item) => (
    <div class="w3-row-padding w3-center w3-margin-top" key={item.id} className="card">
      <div class="w3-third"></div>
      {/* <div class="w3-card w3-container" style={{height:"460px"}}></div> */}
      <h2>{`${item.name}`}</h2>
      <h2>{`Price: $${item.price}`}</h2>
      <img class="w3-card w3-container" src={item.image} style={{ width: "200px", height: "300px", objectFit: "cover" }} className="book-avatar" />
