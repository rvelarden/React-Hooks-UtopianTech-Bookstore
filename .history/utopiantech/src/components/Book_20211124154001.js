...
const Book = (props) => {
    ...
    const updateLike = useCallback(
        ...
        (props) => {
            setItems(props.books.map(indItem => {
            if (indItem !== props.books) {
              return indItem
            } else {
              return {...props.books, likes: props.books.likes + 1}
            }
          }))
      },
    ...
    //   ))
      ...
      return(
            ...
            <div>
            <button disable={props.book.disable} onClick={()=>  setItems(items.map(indItem => {
            if (indItem !== item) {
              return indItem
            } else {
              return {...item, likes: props.books.likes + 1}
            }
          }))
          }> {"Like"} </button>
