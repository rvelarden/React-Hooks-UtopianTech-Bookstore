...
const Shop = () => {
  ...
  const updateLike = useCallback(
    ...
    setText(likeState => ({...likeState, text: !likeState.text}))
    ...
    [items], // Tells React to memoize regardless of arguments.
  );
  ...
  const listItemsToBuy = () => items.map((book) => (
    <div key={book.id} className="card">
      ...
      <div>
      <button id="myBtn" disabled={items.disabled} onClick={()=> updateLike(book)}> {text ? "Liked!" : "Like"} </button>
