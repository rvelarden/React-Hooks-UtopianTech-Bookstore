...
const Shop = () => {
  ...
  setItems(items.map(indItem => {
    setLikes(book => ({
      stateLike: book.stateLike
  }))
  displayText = () =>{
    let display =  !this.state.stateLike
    this.setState({stateLike: display})
    // console.log('clicked')
  }
  ...
  const listItemsToBuy = () => items.map((book) => (
    <div key={book.id} className="card">
      ...
      <div>
      <button id="myBtn" type='button' onClick={()=> updateLike(book)}> {stateLike ? "Liked!" : "Like"} </button>
