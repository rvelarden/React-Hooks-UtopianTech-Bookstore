...
const BookCard = (props)=>{
    ...
    const updateLike = useCallback(
      ...
      setText(likeState => ({...likeState, text: console.log(!likeState)}))
        ...
        if (indItem !== props) {
          ...
          return {...props.item, likes: props.item.likes + 1}
        }
      }))
      },
    ...
    const listItemsToBuy = () => props.map((book) => (
      <div key={props.book.id} className="card">
        ...
        <img src={props.book.image} style={{ width: "200px", height: "300px", objectFit: "cover" }} className="props.book-avatar" />
        <h2>Likes: {props.book.likes}</h2>
        <div>
        <button disable={props.disable} text={props.text} onClick={()=> updateLike(props.book)}> {props.book.liked ? "Liked!" : "Like"} </button>
